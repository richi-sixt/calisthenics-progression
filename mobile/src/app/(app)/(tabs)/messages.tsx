    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-gray-900" edges={["top"]}>
      <View className="flex-1 px-4">
        <View className="flex-row items-center justify-between py-4">
          <Text className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t("messages.title")}</Text>
          <Pressable onPress={() => router.push("/messages/new")} className="rounded-md bg-blue-600 px-4 py-2">
            <Text className="text-sm font-medium text-white">{t("messages.new")}</Text>
          </Pressable>
        </View>
        ...
        {!isLoading && !error && messages.length === 0 && <Text className="mt-6 text-gray-500 dark:text-gray-400">{t("messages.empty")}</Text>}
