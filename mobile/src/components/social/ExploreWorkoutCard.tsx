  ...
  return (
    <View className="rounded-lg border border-gray-200 bg-white p-4">
      <View className="flex-row items-start justify-between">
        <View className="flex-1 flex-row gap-3">
            ...
            <Pressable onPress={() => router.push(`/users/${workout.username}`)}>
              {profilePicUrl ? (
                ...
                <View className="h-10 w-10 items-center justify-center rounded-full bg-gray-200">
                  <Text className="text-sm font-bold text-gray-500">{workout.username[0]?.toUpperCase() ?? "?"}</Text>
                </View>
              )}
            </Pressable>
          )}
          <View className="flex-1">
            ...
            <View className="mt-1 flex-row flex-wrap items-center gap-2">
              {workout.username && (
                <Pressable onPress={() => router.push(`/users/${workout.username}`)}>
                  <Text className="text-sm font-medium text-blue-600">@{workout.username}</Text>
                </Pressable>
              )}
              {workout.timestamp && <Text className="text-sm text-gray-500">{new Date(workout.timestamp).toLocaleDateString()}</Text>}
              <Text className="text-sm text-gray-500">{exerciseCount} {exerciseCount === 1 ? "exercise" : "exercises"}</Text>
            </View>
        ...
        {workout.is_done && (
          <View className="ml-4 rounded-full bg-green-100 px-2.5 py-0.5">
