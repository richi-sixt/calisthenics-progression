...
function formatSetDetails(exercise: Exercise, t: (key: TranslationKey) => string): string | null {
      ...
      <View className="flex-row items-start justify-between">
            ...
            {workout.exercises && workout.exercises.length > 0 && (
              <View className="mt-2 gap-0.5">
                {workout.exercises.map((ex, i) => {
                  const sets = ex.sets ?? [];
                  const setDetails = formatSetDetails(ex, t);
                  return (
        ...
        {workout.is_done && (
          <View className="ml-4 rounded-full bg-green-100 dark:bg-green-900/30 px-2.5 py-0.5">
            <Text className="text-xs font-medium text-green-700 dark:text-green-400">{t("workouts.done")}</Text>
          </View>
        )}
      </View>
    </View>
