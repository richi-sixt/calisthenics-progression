      ...
      <View className="flex-row items-center gap-4">
        ...
        <View className="flex-1">
          ...
          <View className="mt-2 flex-row items-center gap-4">
            <Text className="text-sm text-gray-500 dark:text-gray-400"><Text className="font-bold text-gray-900 dark:text-gray-100">{user.follower_count}</Text> followers</Text>
            <Text className="text-sm text-gray-500 dark:text-gray-400"><Text className="font-bold text-gray-900 dark:text-gray-100">{user.following_count}</Text> following</Text>
          </View>
          {user.last_seen && <Text className="mt-1 text-xs text-gray-400 dark:text-gray-500">Last seen {new Date(user.last_seen).toLocaleDateString()}</Text>}
        </View>
      </View>
      ...
      <View className="mt-3 flex-row flex-wrap gap-2">
        {user.is_following ? (
          <Pressable onPress={() => unfollow.mutate(user.username)} disabled={unfollow.isPending} className="rounded-md bg-gray-100 dark:bg-gray-700 px-3 py-1.5">
            <Text className="text-xs font-medium text-gray-600 dark:text-gray-400">{unfollow.isPending ? "Unfollowing..." : "Unfollow"}</Text>
          </Pressable>
        ) : (
          <Pressable onPress={() => follow.mutate(user.username)} disabled={follow.isPending} className="rounded-md bg-blue-50 dark:bg-blue-900/20 px-3 py-1.5">
            <Text className="text-xs font-medium text-blue-600 dark:text-blue-400">{follow.isPending ? "Following..." : "Follow"}</Text>
          </Pressable>
        )}
        <Pressable
        ...
        >
          <Text className="text-xs font-medium text-gray-600 dark:text-gray-400">Send message</Text>
        </Pressable>
      </View>
    </View>
  );
