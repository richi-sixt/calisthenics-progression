    ...
    <View className="rounded-lg border border-gray-200 bg-white p-4">
      <View className="flex-row gap-3">
        ...
        <Pressable className="flex-1" onPress={() => router.push(`/exercises/${exercise.id}`)}>
          <Text className="text-lg font-semibold text-gray-900">{exercise.title}</Text>
          <View className="mt-1 flex-row flex-wrap items-center gap-2">
            {exercise.username && <Text className="text-sm font-medium text-blue-600">{exercise.username}</Text>}
            ...
            {categoryNames.map((name) => (
              <View key={name} className="rounded-full bg-blue-50 px-2.5 py-0.5">
                <Text className="text-xs font-medium text-blue-600">{name}</Text>
              </View>
            ))}
          </View>
      ...
      <View className="mt-3 flex-row flex-wrap gap-2">
        {isOwner ? (
          <>
            <Pressable onPress={() => router.push(`/exercises/${exercise.id}/edit`)} className="rounded-md bg-gray-100 px-3 py-1.5">
          ...
          <Pressable onPress={() => copyExercise.mutate(exercise.id)} disabled={copyExercise.isPending} className="rounded-md bg-blue-50 px-3 py-1.5">
            <Text className="text-xs font-medium text-blue-600">Copy</Text>
          </Pressable>
        )}
      </View>
    </View>
